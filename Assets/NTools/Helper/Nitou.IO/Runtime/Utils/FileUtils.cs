...
namespace Nitou.IO {
    ...
    public static class FileUtils {
        ...
        /// ファイルの存在チェック．
        /// </summary>
        public static void ExistsWithException(string path) {
            if (!File.Exists(path)) {
        ...
        /// ファイルの存在チェック．
        /// </summary>
        public static void ExistsWithException(IEnumerable<string> paths) {
            foreach (var path in paths)
                ExistsWithException(path);
        }
        #endregion
        ...
        public static List<string> Find(string directoryPath, Predicate<string> filter) {
            DirectoryUtils.ExistsWithException(directoryPath);
